...
# --- Initial Setup ---
mkdir -p logs
mkdir -p "$OUTPUT_BIN_DIR"   # <<< создаем папку для бинаря
...
if [ -z "$CACHED_VOL_ROOT" ]; then
    ...
    if [ "$USE_PREBUILT" = true ]; then
        ...
        docker compose -f compose.yaml -f compose.prebuilt.yaml build build_triedb build_genesis monad_execution monad_node monad_rpc
    else
    	...
    	docker compose build build_triedb build_genesis monad_execution monad_node monad_rpc
    fi
...
# копирование не работает пока
CONTAINER_ID=$(docker create monad-rpc:latest)   # <<< создаем контейнер без запуска
#docker run --rm -v $(pwd)/build:/out monad-rpc:latest cp /usr/local/bin/monad-rpc /out/
docker cp "$CONTAINER_ID:/usr/local/bin/monad-rpc" "$OUTPUT_BIN_DIR/"   # <<< копируем бинарь
docker rm "$CONTAINER_ID"

echo "Binary copied to $OUTPUT_BIN_DIR/monad-rpc"
exit 0